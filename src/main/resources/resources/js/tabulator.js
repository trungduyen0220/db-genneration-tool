...
;(function (global, factory) {
	...
	ColumnManager.prototype._verticalAlignHeaders = function () {
		...
		self.columns.forEach(function (column) {
			...
			// PIONET customize 2020/02/19 START
			if(column.definition.titleFormatter === 'DoubleRow'){
				column.verticalAlign('top', minHeight);
			}else{
				column.verticalAlign(self.table.options.columnHeaderVertAlign, minHeight);
			}
			// PIONET customize 2020/02/19 END
		});
	...
	Column.prototype._buildColumnHeader = function () {
		...
		if (def.field) {
			// PIONET customize 2020/02/19 START
			if(def.titleFormatter !== 'DoubleRow'){
	...
	Cell.prototype._generateContents = function () {
		...
		if (this.table.modExists("format")) {
			// PIONET customize 2020/02/19 START
			if (this.column.definition.titleFormatter === 'DoubleRow') {
				this.element.style.paddingLeft = '0px';
				this.element.style.paddingRight= '0px';
				var value_Top;
				var value_Bottom;
				if(this.column.definition.titleFormatterParams.Formatter_Top_Flg){
					value_Top = this.table.modules.format.formatValue(this, 'Top');
				}else{
					value_Top = this.row.data[this.column.definition.titleFormatterParams.field_Top];
				}
				if(this.column.definition.titleFormatterParams.Formatter_Bottom_Flg){
					value_Bottom = this.table.modules.format.formatValue(this, 'Bottom');
				}else{
					value_Bottom = this.row.data[this.column.definition.titleFormatterParams.field_Bottom];
				}
				val = 
					"<div class='tabulator-cell' role='gridcell' title='' style='width: 100%; border-right-width: 0px;height: 50%;border-bottom: 1px solid #C0C0C0;'>" +
					value_Top +
					'</div><br>' +
					"<div class='tabulator-cell' role='gridcell' title='' style='width: 100%; border-right-width: 0px;height: 50%;'>" +
					value_Bottom +
					'</div>';
			}else{
	...
	Cell.prototype._generateTooltip = function () {
		...
		if (tooltip) {
			...
			// PIONET customize 2020/02/19 START
			if (this.column.definition.titleFormatter === 'DoubleRow') {
				if (this.column.definition.tooltip_Top) {
					var value_Top = this.row.data[this.column.definition.titleFormatterParams.field_Top];
					this.element.childNodes[0].title = value_Top;
				} else {
					this.element.childNodes[0].title = '';
				}
				if (this.column.definition.tooltip_Bottom) {
					var value_Bottom = this.row.data[this.column.definition.titleFormatterParams.field_Bottom];
					this.element.childNodes[2].title = value_Bottom;
				} else {
					this.element.childNodes[2].title = '';
				}
			}else{
	...
	Format.prototype.initializeColumn = function (column) {
		...
		// PIONET customize 2020/02/19 START
		if(column.definition.titleFormatter === 'DoubleRow') {
			switch (_typeof(column.definition.formatter_Top)) {
			case 'string':
				if (column.definition.formatter_Top === 'tick') {
					column.definition.formatter_Top = 'tickCross';
					if (typeof config.params.crossElement === 'undefined') {
						config.params.crossElement = false;
					}
					console.warn('DEPRECATION WARNING - the tick formatter has been deprecated, please use the tickCross formatter with the crossElement param set to false');
				}
				if (self.formatter_Top[column.definition.formatter_Top]) {
					config.formatter_Top = self.formatters[column.definition.formatter_Top];
				} else {
					console.warn('Formatter Error - No such formatter found: ', column.definition.formatter_Top);
					config.formatter_Top = self.formatters.plaintext;
				}
				break;
				
			case 'function':
				config.formatter_Top = column.definition.formatter_Top;
				break;
			...
			switch (_typeof(column.definition.formatter_Bottom)) {
			case 'string':
				if (column.definition.formatter_Bottom === 'tick') {
					column.definition.formatter_Bottom = 'tickCross';
					if (typeof config.params.crossElement === 'undefined') {
						config.params.crossElement = false;
					}
					console.warn('DEPRECATION WARNING - the tick formatter has been deprecated, please use the tickCross formatter with the crossElement param set to false');
				}
				if (self.formatter_Bottom[column.definition.formatter_Bottom]) {
					config.formatter_Bottom = self.formatters[column.definition.formatter_Bottom];
				} else {
					console.warn('Formatter Error - No such formatter found: ', column.definition.formatter_Bottom);
					config.formatter_Bottom = self.formatters.plaintext;
				}
				break;
				
			case 'function':
				config.formatter_Bottom = column.definition.formatter_Bottom;
				break;
	...
	//return a formatted value for a cell
	// PIONET customize 2020/02/19 START
	Format.prototype.formatValue = function (cell,type) {
		...
		if(type === 'Top') {
			return cell.column.modules.format.formatter_Top.call(this, component, params, onRendered);
		}if(type === 'Bottom') {
			return cell.column.modules.format.formatter_Bottom.call(this, component, params, onRendered);
		}else{
			return cell.column.modules.format.formatter.call(this, component, params, onRendered);
		}
	};
	Format.prototype.format_Top_Value = function(cell) {
		var component = cell.getComponent();
		var params = typeof cell.column.modules.format.params === 'function' ? cell.column.modules.format.params(component) : cell.column.modules.format.params;
	...
	Format.prototype.format_bottom_Value = function(cell) {
		var component = cell.getComponent();
		var params = typeof cell.column.modules.format.params === 'function' ? cell.column.modules.format.params(component) : cell.column.modules.format.params;
	...
	var Page = function Page(table) {
		...
		// PIONET customize 2020/03/17 START
		// 総行数
		this.totalRowCount = 0;
		// PIONET customize 2020/03/17 END
	...
	Page.prototype.createElements = function () {
		...
		this.pageInfoElement.classList.add("tabulator-page-info");
		// PIONET customize 2020/03/17 END
	...
	Page.prototype.initialize = function (hidden) {
		...
		// PIONET customize 2020/03/17 START
		self.element.appendChild(self.pageInfoElement); // ページ情報
		// PIONET customize 2020/03/17 END
	...
	Page.prototype._setPageButtons = function () {
		...
		// PIONET customize 2020/03/17 START
		var start = (this.page - 1) * this.size + 1;
		var end = this.page * this.size < this.totalRowCount ? this.page * this.size : this.totalRowCount;
		// PIONET customize 2020/03/17 END
		...
		// PIONET customize 2020/03/17 START
		while (self.pageInfoElement.firstChild) {
			self.pageInfoElement.removeChild(self.pageInfoElement.firstChild);
		}
		// PIONET customize 2020/03/17 END
		if (self.page == 1) {
		...
		// pagesElement作成
		var leftSeparatorLabel = document.createElement("span");
		leftSeparatorLabel.classList.add("tabulator-page-area-separator");
		leftSeparatorLabel.appendChild(document.createTextNode("|"));

		var pageLabel = document.createElement("span");
		pageLabel.classList.add("tabulator-page-area-label");
		pageLabel.appendChild(document.createTextNode("ページ"));

		var pageTextBox = document.createElement("input");
		pageTextBox.classList.add("tabulator-page-area-input");
		pageTextBox.setAttribute("type","text");
		pageTextBox.setAttribute("size","3");
		pageTextBox.setAttribute("maxlength","3");
		pageTextBox.setAttribute("value",self.page);
		
		pageTextBox.addEventListener('blur', function(e) {
			var inputPage = e.target.value;
			if (0 < inputPage && inputPage <= self.max && inputPage !== self.page) {
				self.setPage(inputPage);
			} else {
				e.target.value = self.page;
			}
		});
		
		var slashLabel = document.createElement("span");
		slashLabel.classList.add("tabulator-page-area-label");
		slashLabel.appendChild(document.createTextNode("/"));
		
		var maxPageLabel = document.createElement("span");
		maxPageLabel.classList.add("tabulator-page-area-label");
		maxPageLabel.appendChild(document.createTextNode(self.max));

		var rightSeparatorLabel = document.createElement("span");
		rightSeparatorLabel.classList.add("tabulator-page-area-separator");
		rightSeparatorLabel.appendChild(document.createTextNode("|"));

		self.pagesElement.appendChild(leftSeparatorLabel);
		self.pagesElement.appendChild(pageLabel);
		self.pagesElement.appendChild(pageTextBox);
		self.pagesElement.appendChild(slashLabel);
		self.pagesElement.appendChild(maxPageLabel);
		self.pagesElement.appendChild(rightSeparatorLabel);
		
		// ページ情報
		var pageInfoLabel = document.createElement("span");
		pageInfoLabel.classList.add("tabulator-page-info-label");
		pageInfoLabel.appendChild(document.createTextNode(this.totalRowCount + '件中 ' + start + ' - ' + end + ' を表示'));
		
		self.pageInfoElement.appendChild(pageInfoLabel);
		// PIONET customize 2020/03/17 END
	...
	Page.prototype._parseRemoteData = function (data) {
		...
		// PIONET customize 2020/03/17 START
		// last_page（最終ページ）を受け取る代わりに、総行数を受け取るように修正する。ページング情報に表示が必要なため。
...
//		}
		...
		if (typeof data[this.paginationDataReceivedNames.total_row_count] === "undefined") {
			console.warn("Remote Pagination Error - Server response missing '" + this.paginationDataReceivedNames.total_row_count + "' property");
		}

		if (data[this.paginationDataReceivedNames.data]) {
//			this.max = parseInt(data[this.paginationDataReceivedNames.last_page]) || 1;
			this.totalRowCount = parseInt(data[this.paginationDataReceivedNames.total_row_count]) || 0;
			this.max = Math.ceil(this.totalRowCount / this.size);
			// PIONET customize 2020/03/17 END
	...
	Sort.prototype.initializeColumn = function (column, content) {
		...
		// PIONET customize 2020/02/19 START
		if (column.definition.titleFormatter === 'DoubleRow') {
			var title_Top = column.definition.titleFormatterParams.title_Top;
			var title_Bottom = column.definition.titleFormatterParams.title_Bottom;
			
			// Top
			var colElDoubleTop = document.createElement('div');
			colElDoubleTop.classList.add('tabulator-col');
			colElDoubleTop.classList.add('tabulator-sortable');
			colElDoubleTop.classList.add('tabulator-doublerow-top');
			colElDoubleTop.setAttribute('aria-sort', 'none');
			colElDoubleTop.setAttribute('tabulator-field', column.definition.titleFormatterParams.field_Top);
			
			var colContentDoubleTop = document.createElement('div');
			colContentDoubleTop.classList.add('tabulator-col-content');
			
			var colTitleDoubleTop = document.createElement('div');
			colTitleDoubleTop.classList.add('tabulator-col-title');
			
			var textNodeTitleTop = document.createTextNode(title_Top);
			colTitleDoubleTop.appendChild(textNodeTitleTop);

			var arrowElDoubleTop = document.createElement('div');
			arrowElDoubleTop.classList.add('tabulator-arrow');
			
			colContentDoubleTop.appendChild(colTitleDoubleTop);
			colContentDoubleTop.appendChild(arrowElDoubleTop);
			
			colElDoubleTop.appendChild(colContentDoubleTop);

			// Bottom
			var colElDoubleBottom = document.createElement('div');
			colElDoubleBottom.classList.add('tabulator-col');
			colElDoubleBottom.classList.add('tabulator-sortable');
			colElDoubleBottom.classList.add('tabulator-doublerow-bottom');
			colElDoubleBottom.setAttribute('aria-sort', 'none');
			colElDoubleBottom.setAttribute('tabulator-field', column.definition.titleFormatterParams.field_Bottom);
			
			var colContentDoubleBottom = document.createElement('div');
			colContentDoubleBottom.classList.add('tabulator-col-content');
			
			var colTitleDoubleBottom = document.createElement('div');
			colTitleDoubleBottom.classList.add('tabulator-col-title');
			
			var textNodeTitleBottom = document.createTextNode(title_Bottom);
			colTitleDoubleBottom.appendChild(textNodeTitleBottom);

			var arrowElDoubleBottom = document.createElement('div');
			arrowElDoubleBottom.classList.add('tabulator-arrow');
			
			colContentDoubleBottom.appendChild(colTitleDoubleBottom);
			colContentDoubleBottom.appendChild(arrowElDoubleBottom);
			
			colElDoubleBottom.appendChild(colContentDoubleBottom);
			
			// 設定済みの内容を削除
			if (column.getElement().hasChildNodes()) {
				column.getElement().removeChild(column.getElement().firstChild);
			}
			column.getElement().appendChild(colElDoubleTop);
			column.getElement().appendChild(document.createElement('br'));
			column.getElement().appendChild(colElDoubleBottom);
			
			//sort on click
			colElDoubleTop.addEventListener("click", function (e) {
				if (colElDoubleTop.getAttribute('tabulator-field') != null) {
					var currentSorters = table.getSorters();
					var field = colElDoubleTop.getAttribute('tabulator-field');
					var dir = "asc";
					if (currentSorters.length > 0 && currentSorters[0].field === colElDoubleTop.getAttribute('tabulator-field')) {
						switch (currentSorters[0].dir) {
							case "asc":
								dir = "desc";
								break;
			...
			colElDoubleBottom.addEventListener("click", function (e) {
				if (colElDoubleBottom.getAttribute('tabulator-field') != null) {
					var currentSorters = table.getSorters();
					var field = colElDoubleBottom.getAttribute('tabulator-field');
					var dir = "asc";
					if (currentSorters.length > 0 && currentSorters[0].field === colElDoubleBottom.getAttribute('tabulator-field')) {
						switch (currentSorters[0].dir) {
							case "asc":
								dir = "desc";
								break;
		...
		} else if (typeof column.definition.headerSort === "undefined" ? this.table.options.headerSort !== false : column.definition.headerSort !== false) {
			// PIONET customize 2020/02/19 END
			colEl = column.getElement();
	...
	Sort.prototype.sort = function (data) {
		...
		if (!self.table.options.ajaxSorting) {
			...
			sortList.forEach(function (item, i) {
				...
				// PIONET customize 2020/03/16 START
//				self.setColumnHeader(item.column, item.dir);
				self.table.columnManager.getRealColumns().forEach(function(column) {
					if (column.definition.titleFormatter === 'DoubleRow') {
						if (column.element.children[0].getAttribute('tabulator-field') === item.column.field) {
							column.element.children[0].setAttribute('aria-sort', item.dir);
						} else if (column.element.children[2].getAttribute('tabulator-field') === item.column.field) {
							column.element.children[2].setAttribute('aria-sort', item.dir);
						}
					} else if (column.field === item.column.field) {
						column.modules.sort.dir = item.dir;
						column.getElement().setAttribute('aria-sort', item.dir);
					}
				});
				// PIONET customize 2020/03/16 END
			});
		} else {
			sortList.forEach(function (item, i) {
				// PIONET customize 2020/03/16 START
	...
	Sort.prototype.clearColumnHeaders = function () {
		this.table.columnManager.getRealColumns().forEach(function (column) {
			if (column.modules.sort) {
				...
				// PIONET customize 2020/03/16 START
				if (column.definition.titleFormatter === 'DoubleRow' && column.element.children.length > 1) {
					column.element.children[0].setAttribute('aria-sort', 'none');
					column.element.children[2].setAttribute('aria-sort', 'none');
				}
				// PIONET customize 2020/03/16 END
			}
		});
